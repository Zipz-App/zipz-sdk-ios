...
extension User
{
    ...
    static func fetch(with uuid: String) -> User?
    {
        ...
        do  { user = try context().fetch(request).first }
            ...
            debugLog("🧨 Error fetching USER with ID \(uuid): \(error.userInfo)")
        }
        return user
    }
    
    static func authorized() -> User?
    {
        request.returnsObjectsAsFaults = false
        ...
        do  { user = try context().fetch(request).first }
            ...
            debugLog("🧨 Error fetching authorized USER: \(error.userInfo)")
        }
        return user
    }
}
