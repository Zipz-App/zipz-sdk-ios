...
extension Offer
{
    ...
    static func fetch(with uuid: String) -> Offer?
    {
        ...
        do  { offer = try context().fetch(request).first }
            ...
            debugLog("🧨 Error fetching OFFER with UUID \(uuid): \(error.userInfo)")
        }
        return offer
    }
    
    static func all() -> [Offer]?
    {
        request.returnsObjectsAsFaults = false
        ...
        do  { offers = try context().fetch(request) }
            ...
            debugLog("🧨 Error fetching all OFFERS: \(error.userInfo)")
        }
        return offers
    }
}
