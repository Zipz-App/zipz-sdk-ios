...
extension Venue
{
    ...
    public static func fetch(with uuid: String) -> Venue?
    {
        ...
        do  { venue = try context().fetch(request).first }
            ...
            ZipzSDK.debugLog("🧨 Error fetching VENUE with UUID \(uuid): \(error.userInfo)")
        }
        return venue
    }
    
    public static func all() -> [Venue]?
    {
        request.returnsObjectsAsFaults = false
        ...
        do  { venues = try context().fetch(request) }
            ...
            ZipzSDK.debugLog("🧨 Error fetching all VENUES: \(error.userInfo)")
        }
        return venues
    }
}
