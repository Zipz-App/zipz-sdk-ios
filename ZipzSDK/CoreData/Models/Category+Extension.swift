...
extension Category
{
    ...
    static func fetch(with name: String) -> Category?
    {
        ...
        do  { object = try context().fetch(request).first }
            ...
            ZipzSDK.debugLog("🧨 Error fetching CATEGORY with NAME \(name): \(error.userInfo)")
        }
        return object
    }
    
    public static func all() -> [Category]?
    {
        request.returnsObjectsAsFaults = false
        ...
        do  { objects = try context().fetch(request) }
            ...
            ZipzSDK.debugLog("🧨 Error fetching all CATEGORIES: \(error.userInfo)")
        }
        return objects
    }
}
